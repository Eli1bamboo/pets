            ...
            <div className="flex-1 overflow-y-auto space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="px-4 py-3 bg-gray-50/50 border-b border-gray-100 flex items-center gap-2 text-gray-900 font-bold text-sm">
                        <Calendar className="text-admin-accent" size={16} />
                        Horarios Disponibles
                    </div>
                    <div className="divide-y divide-gray-100">
                        {settingsLoading ? (
                            <div className="p-8 text-center text-admin-text-secondary text-sm">Cargando horarios...</div>
                        ) : (
                            localHours.map((bh) => (
                                <div key={bh.id} className={`p-4 flex flex-col gap-3 transition-colors ${!bh.is_active ? 'bg-gray-50' : ''}`}>
                                    <div className="flex items-center justify-between">
                                        <span className={`font-bold ${bh.is_active ? 'text-admin-primary' : 'text-gray-500'}`}>
                                            {DAYS_NAMES[bh.day_of_week]}
                                        </span>
                                        <button
                                            onClick={() => handleToggle(bh.id)}
                                            className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-admin-accent focus:ring-offset-2 ${bh.is_active ? 'bg-admin-accent' : 'bg-gray-200'}`}
                                        >
                                        ...
                                        <div className="grid grid-cols-2 gap-3 pl-1">
                                            <div className="space-y-1">
                                                ...
                                                <input
                                                    ...
                                                    className="block w-full rounded-lg border-gray-200 text-sm focus:border-admin-accent focus:ring-admin-accent py-1.5"
                                                />
                                            </div>
                                            <div className="space-y-1">
